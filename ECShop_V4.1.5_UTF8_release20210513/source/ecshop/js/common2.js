...
/* JS代码增加_start  By www.fd30.com */
function reg_package(){
  var pal=document.getElementById("package_tit").getElementsByTagName("li");
  ...
  for(var i=0;i<pal.length;i++){
    ...
    pal[i].onclick=function(){      
      for(var j=0;j<pal_count;j++){
        var _pal=document.getElementById("package_tit").getElementsByTagName("li")[j];
        var ison=j==this.pai;
        _pal.className=(ison?"selected":"");
      }
      for(var j=0;j<pal_count;j++){
        var _palb=document.getElementById("package_box_"+j);
        var ison=j==this.pai;
        _palb.style.display=(ison?"block":"none");
      }
    }
  }
...
/* 组合购买__添加商品到购物车__Start     By www.fd30.com  */
...
/* 组合购买__添加商品到购物车__End     By www.fd30.com */
...
/* 代码修改_start By www.fd30.com  这里增加了一个变量 */
...
function openSpeDiv(message, goods_id, parent) 
{
  ...
  {
      newDiv.innerHTML += '<h6 >' +  message[spec]['name'] + '</h6>';
      ...
      if (message[spec]['attr_type'] == 1)
      {
        for (var val_arr = 0; val_arr < message[spec]['values'].length; val_arr++)
        {
          if (val_arr == 0)
          {
            newDiv.innerHTML += "<label><input type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' checked /><font>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font></label>';      
          }
          else
          {
            newDiv.innerHTML += "<label><input type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' /><font>" + message[spec]['values'][val_arr]['label'] + '</font> [' + message[spec]['values'][val_arr]['format_price'] + ']</font></label>';      
          }
        } 
      ...
      else
      {
        for (var val_arr = 0; val_arr < message[spec]['values'].length; val_arr++)
        {
          newDiv.innerHTML += "<label><input type='checkbox' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' /><font>" + message[spec]['values'][val_arr]['label'] + ' [' + message[spec]['values'][val_arr]['format_price'] + ']</font></label>';     
        }
        newDiv.innerHTML += "<input type='hidden' name='spec_list' value='" + val_arr + "' />";
      }
  }
  newDiv.innerHTML +="<center><a href='javascript:submit_div(" + goods_id + "," + parent + ")' class='f6' >" + btn_buy + "</a><a href='javascript:cancel_div()' class='f7' >" + is_cancel + "</a></center>";
...
/*newDiv.style.top = (parseInt(scrollPos + 20)) + "px";*/
newDiv.style.top ="260px";
newDiv.style.left = (parseInt(document.body.offsetWidth) - 300) / 2 + "px"; // 屏幕居中
newDiv.style.background = "#fff";
newDiv.style.border = "1px solid #ccc";
var html = '';
...
//生成层内内容
html = '<div class="conclose"><span>添加成功！</span></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id='+goods_id+'" class="pic"><img src='+pic+' width="100" height="100"/></a><p><span>'+name+'</span><font class="word_c1">'+price+'</font></p><div class="clear0"></div></div>';
html += '<div class="coninfo">';
html +='购物车共有<strong class="word_c1">'+number+'</strong>种商品，合计：<strong  class="word_c">'+total+'</strong>';
html +='</div>';

html +="<div class='fd30_btn'><a href=\'javascript:cancel_div()\' class='a_word_c'>再逛逛</a><a href='flow.php?step=checkout' class='bg_c fd_submit' >去结算</a></div>";
html +='</div></div>';
