...
class Templater
{
	...
	public function fnReactionTitle($templater, &$escape, $reactionId)
	{
		...
		return \XF::phrase('reaction_title.' . $reactionId);
	}
		...
		if ($user->canViewCurrentActivity() && $user->Activity)
		{
			if ($user->Activity->description)
			{
				...
				if ($user->Activity->view_state == 'error' && \XF::visitor()->canBypassUserPrivacy())
				{
					...
					$activityDetail .= '<i class="fa fa-exclamation-triangle u-muted" title="' . $this->filterForAttr($this,\XF::phrase('viewing_an_error'), $null) . '" aria-hidden="true"></i>';
					$activityDetail .= ' <span class="u-srOnly">' . \XF::phrase('viewing_an_error') . '</span>';
				}
			}
		}
	...
	public function filterFormat($templater, $value, &$escape, $format = '%s')
	{
	...
	{
		return $this->language->shortNumberFormat($value, $precision);
	}
	...
	{
		return $this->app->stringFormatter()->fromCamelCase($value, $glue);
	}
	...
	public function formDateInput(array $controlOptions)
	{
		...
		return "<div class=\"inputGroup inputGroup--date inputGroup--joined inputDate\"><input type=\"text\" class=\"input input--date {$class}\" autocomplete=\"off\"{$xfInitAttr}{$weekStartAttr}{$readOnlyAttr}{$unhandledAttrs} /><span class=\"inputGroup-text inputDate-icon js-dateTrigger\"></span></div>";
	}
	...
	public function formEditor(array $controlOptions)
	{
		...
		{
			$dropdown['title'] = \XF::phrase('editor_dropdown.' . $cmd);
		}
	...
	public function formTextBox(array $controlOptions)
	{
		...
		$input = "{$fa}<input{$typeAttr} class=\"" . trim("input {$class}") . "\"{$xfInitAttr}{$acSingle}{$readOnlyAttr}{$unhandledAttrs} />";
	...
	public function formNumberBox(array $controlOptions)
	{
		...
		$input = "<div class=\"inputGroup inputGroup--numbers inputNumber\" data-xf-init=\"number-box\"{$buttonSmallerAttr}{$stepOverrideAttr}>"
	...
	public function renderWidgetClosure(\Closure $widgetHandler, array $options = [])
	{
		...
		try
		{
			$output = $widgetHandler($this, $this->defaultParams, $options);
		}
		catch (\Exception $e)
		{
			if (\XF::$debugMode)
